...
export default function TradeDialog({ isOpen, onOpenChange, item, tradeType, playerStats, inventory, onTrade }: TradeDialogProps) {
  ...
  useEffect(() => {
    ...
    if (isNaN(amount) || amount <= 0) {
        ...
        else if (totalCost > playerStats.netWorth) error = `Insufficient funds. You need ${formatAbbreviatedNumber(totalCost)}¢.`;
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="space-y-2 rounded-md border border-border/50 p-3 bg-card/50">
            <div className="flex justify-between items-center text-sm">
              <p className="flex items-center gap-2 text-muted-foreground"><Coins className="h-4 w-4 text-amber-400" /> Total Price ({formatAbbreviatedNumber(effectivePrice)}¢ each):</p>
              <p className={`font-mono ${tradeType === 'buy' && totalPrice > playerStats.netWorth ? 'text-destructive' : 'text-foreground'}`}>
                {formatAbbreviatedNumber(totalPrice)} ¢
              </p>
            </div>
            <div className="flex justify-between items-center text-sm">
