...
interface BarValueChartProps {
  valueHistory: number[];
}

export default function BarValueChart({ valueHistory }: BarValueChartProps) {
        ...
        {chartData.length > 1 ? (
          <ChartContainer config={chartConfig} className="h-[150px] w-full">
            <LineChart data={chartData} margin={{ top: 5, right: 20, left: -10, bottom: 0 }}>
              ...
              <YAxis domain={['dataMin - 500', 'dataMax + 500']} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }} tickLine={false} axisLine={false} tickFormatter={(value) => `¢${formatAbbreviatedNumber(Number(value))}`} />
              <ChartTooltip
