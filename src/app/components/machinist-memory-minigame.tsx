...
export default function MachinistMemoryMinigame() {
  const { handleMachinistMinigameScore } = useGame();
  ...
  const handlePlayerClick = (index: number) => {
    ...
    if (newPlayerInput[newPlayerInput.length - 1] !== sequence[newPlayerInput.length - 1]) {
      ...
      handleMachinistMinigameScore(score);
      return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Match reactor calibration sequences under pressure to avoid factory malfunction. Each successful sequence increases your score. One mistake ends the simulation.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {gameState === GameState.Idle && (
            <Button onClick={handleStartGame} className="w-full">
                <Zap className="mr-2"/> Start Calibration
            </Button>
        )}
        {gameState === GameState.Finished && (
             <div className="text-center space-y-2">
                <p className="font-bold text-lg">Final Score: {score.toLocaleString()}¢</p>
                <p className="text-muted-foreground text-sm">Credits have been transferred to your account for improving efficiency.</p>
                <Button onClick={handleResetGame} className="w-full">
                    <Zap className="mr-2"/> Run New Calibration
                </Button>
            </div>
        )}
      </CardContent>
