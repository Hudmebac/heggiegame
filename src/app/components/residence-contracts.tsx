...
import ResidenceValueChart from './residence-value-chart';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/components/ui/alert-dialog';
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { runResidencePartnershipOfferGeneration } from '@/app/actions';
import { useToast } from "@/hooks/use-toast";

interface ResidenceContractsProps {
...
export default function ResidenceContracts({ playerStats, onSell, onExpand, canAffordExpansion, expansionButtonLabel, nextExpansionTier }: ResidenceContractsProps) {
    const { handleAcceptResidencePartnerOffer } = useGame();
    ...
    if (!playerStats.residenceContract) return null;
    ...
    const handleFetchOffers = async () => {
        ...
        try {
            const result = await runResidencePartnershipOfferGeneration({ marketValue: playerStats.residenceContract!.currentMarketValue });
    ...
    const handleOfferAccepted = (offer: PartnershipOffer) => {
        handleAcceptResidencePartnerOffer(offer);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Manage ownership and strategic partnerships for this residence.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">Property Level</span>
                    <span className="font-mono">{getEstablishmentLevelLabel(playerStats.residenceEstablishmentLevel)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground flex items-center gap-1.5"><TrendingUp className="h-4 w-4"/> Current Market Value</span>
                    <span className="font-mono text-amber-300">{playerStats.residenceContract.currentMarketValue.toLocaleString()}¢</span>
                </div>

                <div className="space-y-2">
                    <p className="text-xs text-muted-foreground">Value Fluctuation (Last 20 Cycles)</p>
                    <ResidenceValueChart valueHistory={playerStats.residenceContract.valueHistory} />
                </div>
                ...
                <AlertDialog>
                    <AlertDialogTrigger asChild>
                        <Button variant="destructive" className="w-full">Sell Property</Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            <AlertDialogTitle>Are you sure you want to sell?</AlertDialogTitle>
                            <AlertDialogDescription>
                                You will sell the property for its current market value of {playerStats.residenceContract.currentMarketValue.toLocaleString()}¢. All residence levels, bot hires, and expansion progress will be lost. This action is irreversible.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                ...
                <div className="pt-4 border-t border-border/50"></div>
                 ...
                 <div>
                    ...
                    <div className="p-4 rounded-lg bg-background/50 text-sm space-y-2">
                         {playerStats.residenceContract.partners.length > 0 ? (
                            playerStats.residenceContract.partners.map(partner => (
                         ...
                         <Dialog open={isOffersDialogOpen} onOpenChange={setIsOffersDialogOpen}>
                            ...
                            <DialogContent className="max-w-2xl">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Several factions have expressed interest in acquiring a stake in your property. Review their offers carefully.
                                    </DialogDescription>
                                </DialogHeader>
