...
import ConstructionValueChart from './construction-value-chart';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/components/ui/alert-dialog';
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { runConstructionPartnershipOfferGeneration } from '@/app/actions';
import { useToast } from "@/hooks/use-toast";

interface ConstructionContractsProps {
...
const getEstablishmentLevelLabel = (level: number) => {
    ...
    if (level === 5) return 'Galactic Megastructure';
    return `Expansion Level ${level - 1}`;
};

export default function ConstructionContracts({ playerStats, onSell, onExpand, canAffordExpansion, expansionButtonLabel, nextExpansionTier }: ConstructionContractsProps) {
    const { handleAcceptConstructionPartnerOffer } = useGame();
    ...
    if (!playerStats.constructionContract) return null;
    ...
    const handleFetchOffers = async () => {
        ...
        try {
            const result = await runConstructionPartnershipOfferGeneration({ marketValue: playerStats.constructionContract!.currentMarketValue });
    ...
    const handleOfferAccepted = (offer: PartnershipOffer) => {
        handleAcceptConstructionPartnerOffer(offer);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Manage ownership and strategic partnerships for this construction project.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">Project Level</span>
                    <span className="font-mono">{getEstablishmentLevelLabel(playerStats.constructionEstablishmentLevel)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground flex items-center gap-1.5"><TrendingUp className="h-4 w-4"/> Current Market Value</span>
                    <span className="font-mono text-amber-300">{playerStats.constructionContract.currentMarketValue.toLocaleString()}¢</span>
                </div>

                <div className="space-y-2">
                    <p className="text-xs text-muted-foreground">Value Fluctuation (Last 20 Cycles)</p>
                    <ConstructionValueChart valueHistory={playerStats.constructionContract.valueHistory} />
                </div>
                ...
                <AlertDialog>
                    <AlertDialogTrigger asChild>
                        <Button variant="destructive" className="w-full">Sell Project</Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            <AlertDialogTitle>Are you sure you want to sell?</AlertDialogTitle>
                            <AlertDialogDescription>
                                You will sell the construction project for its current market value of {playerStats.constructionContract.currentMarketValue.toLocaleString()}¢. All levels, bot hires, and expansion progress will be lost. This action is irreversible.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                ...
                <div className="pt-4 border-t border-border/50"></div>
                 ...
                 <div>
                    ...
                    <div className="p-4 rounded-lg bg-background/50 text-sm space-y-2">
                         {playerStats.constructionContract.partners.length > 0 ? (
                            playerStats.constructionContract.partners.map(partner => (
                         ...
                         <Dialog open={isOffersDialogOpen} onOpenChange={setIsOffersDialogOpen}>
                            ...
                            <DialogContent className="max-w-2xl">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Several construction guilds have expressed interest in acquiring a stake in your project. Review their offers carefully.
                                    </DialogDescription>
                                </DialogHeader>
