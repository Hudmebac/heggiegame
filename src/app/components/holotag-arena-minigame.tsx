...
export default function HolotagArenaMinigame() {
  const { handleHolotagMinigameScore } = useGame();
  ...
  useEffect(() => {
    ...
    const spawnTimer = setInterval(() => {
      setTargets(prevTargets => {
        ...
        const type: TargetType = Math.random() > 0.3 ? 'enemy' : 'friendly';
  ...
  useEffect(() => {
    if (!isPlaying && gameTime === 0) {
      handleHolotagMinigameScore(score > 0 ? score : 0);
    }
  }, [isPlaying, gameTime, score, handleHolotagMinigameScore]);
  ...
  const getIconForType = (type: TargetType) => {
    if (type === 'enemy') return <Target className="h-6 w-6 text-primary animate-pulse" />;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Tag hostile holograms to earn points. Avoid hitting friendly projections!
        </CardDescription>
      </CardHeader>
