...
import RecreationValueChart from './recreation-value-chart';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/components/ui/alert-dialog';
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { runRecreationPartnershipOfferGeneration } from '@/app/actions';
import { useToast } from "@/hooks/use-toast";

interface RecreationContractsProps {
...
export default function RecreationContracts({ playerStats, onSell, onExpand, canAffordExpansion, expansionButtonLabel, nextExpansionTier }: RecreationContractsProps) {
    const { handleAcceptRecreationPartnerOffer } = useGame();
    ...
    if (!playerStats.recreationContract) return null;
    ...
    const handleFetchOffers = async () => {
        ...
        try {
            const result = await runRecreationPartnershipOfferGeneration({ marketValue: playerStats.recreationContract!.currentMarketValue });
    ...
    const handleOfferAccepted = (offer: PartnershipOffer) => {
        handleAcceptRecreationPartnerOffer(offer);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Manage ownership and strategic partnerships for this recreation facility.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">Facility Level</span>
                    <span className="font-mono">{getEstablishmentLevelLabel(playerStats.recreationEstablishmentLevel)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground flex items-center gap-1.5"><TrendingUp className="h-4 w-4"/> Current Market Value</span>
                    <span className="font-mono text-amber-300">{playerStats.recreationContract.currentMarketValue.toLocaleString()}¢</span>
                </div>

                <div className="space-y-2">
                    <p className="text-xs text-muted-foreground">Value Fluctuation (Last 20 Cycles)</p>
                    <RecreationValueChart valueHistory={playerStats.recreationContract.valueHistory} />
                </div>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                You will sell the recreation facility for its current market value of {playerStats.recreationContract.currentMarketValue.toLocaleString()}¢. All levels, bot hires, and expansion progress will be lost. This action is irreversible.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                ...
                <div className="pt-4 border-t border-border/50"></div>
                 ...
                 <div>
                    ...
                    <div className="p-4 rounded-lg bg-background/50 text-sm space-y-2">
                         {playerStats.recreationContract.partners.length > 0 ? (
                            playerStats.recreationContract.partners.map(partner => (
                         ...
                         <Dialog open={isOffersDialogOpen} onOpenChange={setIsOffersDialogOpen}>
                            ...
                            <DialogContent className="max-w-2xl">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Several entertainment guilds have expressed interest in acquiring a stake in your facility. Review their offers carefully.
                                    </DialogDescription>
                                </DialogHeader>
