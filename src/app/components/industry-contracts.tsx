...
import IndustryValueChart from './industry-value-chart';
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from '@/components/ui/alert-dialog';
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { runIndustryPartnershipOfferGeneration } from '@/app/actions';
import { useToast } from "@/hooks/use-toast";

interface IndustryContractsProps {
...
export default function IndustryContracts({ playerStats, onSell, onExpand, canAffordExpansion, expansionButtonLabel, nextExpansionTier }: IndustryContractsProps) {
    const { handleAcceptIndustryPartnerOffer } = useGame();
    ...
    if (!playerStats.industryContract) return null;
    ...
    const handleFetchOffers = async () => {
        ...
        try {
            const result = await runIndustryPartnershipOfferGeneration({ marketValue: playerStats.industryContract!.currentMarketValue });
    ...
    const handleOfferAccepted = (offer: PartnershipOffer) => {
        handleAcceptIndustryPartnerOffer(offer);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Manage ownership and strategic partnerships for this industrial facility.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground">Facility Level</span>
                    <span className="font-mono">{getEstablishmentLevelLabel(playerStats.industryEstablishmentLevel)}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                    <span className="text-muted-foreground flex items-center gap-1.5"><TrendingUp className="h-4 w-4"/> Current Market Value</span>
                    <span className="font-mono text-amber-300">{playerStats.industryContract.currentMarketValue.toLocaleString()}¢</span>
                </div>

                <div className="space-y-2">
                    <p className="text-xs text-muted-foreground">Value Fluctuation (Last 20 Cycles)</p>
                    <IndustryValueChart valueHistory={playerStats.industryContract.valueHistory} />
                </div>
                    ...
                    <AlertDialogContent>
                        <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                                You will sell the industrial facility for its current market value of {playerStats.industryContract.currentMarketValue.toLocaleString()}¢. All levels, bot hires, and expansion progress will be lost. This action is irreversible.
                            </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                ...
                <div className="pt-4 border-t border-border/50"></div>
                 ...
                 <div>
                    ...
                    <div className="p-4 rounded-lg bg-background/50 text-sm space-y-2">
                         {playerStats.industryContract.partners.length > 0 ? (
                            playerStats.industryContract.partners.map(partner => (
                         ...
                         <Dialog open={isOffersDialogOpen} onOpenChange={setIsOffersDialogOpen}>
                            ...
                            <DialogContent className="max-w-2xl">
                                <DialogHeader>
                                    ...
                                    <DialogDescription>
                                        Several industrial consortiums have expressed interest in acquiring a stake in your facility. Review their offers carefully.
                                    </DialogDescription>
                                </DialogHeader>
