...
export function useResidence(
  ...
  const handleUpgradeResidence = useCallback(() => {
    setGameState(prev => {
      ...
      const upgradeCost = Math.round(188 * 1.5 * Math.pow(prev.playerStats.residenceLevel, 2.5) * costModifier * landlordDiscount);
  ...
  const handleUpgradeResidenceAutoClicker = useCallback(() => {
    setGameState(prev => {
      ...
      const botCost = Math.round(4250 * 1.5 * Math.pow(2.25, prev.playerStats.residenceAutoClickerBots) * costModifier * landlordDiscount);
  ...
  const handlePurchaseResidence = useCallback(() => {
    setGameState(prev => {
        ...
        const landlordDiscount = prev.playerStats.career === 'Landlord' ? 0.8 : 1.0;
        const cost = 5000 * 2.0 * landlordDiscount;
