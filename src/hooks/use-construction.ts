...
export function useConstruction(
  ...
  const handleUpgradeConstruction = useCallback(() => {
    setGameState(prev => {
      ...
      const upgradeCost = calculateCost(playerStats.constructionLevel + 1, upgradeConfig.starterPrice, upgradeConfig.growth, difficultyModifier * costModifier);
  ...
  const handleUpgradeConstructionAutoClicker = useCallback(() => {
    setGameState(prev => {
      ...
      const botCost = calculateCost(playerStats.constructionAutoClickerBots + 1, botConfig.starterPrice, botConfig.growth, difficultyModifier * costModifier);
  ...
  const handleExpandConstruction = useCallback(() => {
    setGameState(prev => {
        ...
        const cost = calculateCost(playerStats.constructionEstablishmentLevel, establishmentConfig.starterPrice, establishmentConfig.growth, difficultyModifier * costModifier);
